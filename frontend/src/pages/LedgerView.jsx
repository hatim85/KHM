  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate(-1)} className="p-2 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white rounded-xl transition">
          ← Back
        </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5">
          ...
          <p className={`text-3xl font-bold font-mono mt-1 ${currentBalance > 0 ? (isCustomer ? 'text-emerald-400' : 'text-rose-400') : currentBalance < 0 ? 'text-amber-400' : 'text-slate-400'}`}>
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5">
          ...
          <p className="text-3xl font-bold text-white font-mono mt-1">{ledger.length}</p>
        </div>
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 flex items-end">
          <select value={streamFilter} onChange={(e) => setStreamFilter(e.target.value)} className="w-full bg-slate-950/60 border border-slate-700/70 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-sm text-white outline-none appearance-none">
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="bg-slate-800/40 border-b border-slate-800">
                ...
                <th className="py-4 px-6 text-xs font-semibold text-slate-400 uppercase tracking-wider text-right">Debit (₹)</th>
                <th className="py-4 px-6 text-xs font-semibold text-slate-400 uppercase tracking-wider text-right">Credit (₹)</th>
                <th className="py-4 px-6 text-xs font-semibold text-slate-400 uppercase tracking-wider text-right">Balance (₹)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/50">
              {loading && ledger.length === 0 ? (
                ...
                ledger.map((entry, idx) => (
                  <tr key={entry._id || idx} className="hover:bg-slate-800/20 transition">
                    <td className="py-4 px-6 text-sm text-slate-300">{new Date(entry.createdAt).toLocaleDateString('en-IN')}</td>
                    <td className="py-4 px-6">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded ${entry.stream === 'TAX' ? 'bg-indigo-500/10 text-indigo-400' : 'bg-amber-500/10 text-amber-400'}`}>
                        {entry.stream}
                      </span>
                    </td>
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6 text-right text-sm font-mono">
                      {entry.debit > 0 ? (
                        <span className="text-rose-400">{(entry.debit / 100).toFixed(2)}</span>
                      ) : <span className="text-slate-600">—</span>}
                    </td>
                    <td className="py-4 px-6 text-right text-sm font-mono">
                      {entry.credit > 0 ? (
                        <span className="text-emerald-400">{(entry.credit / 100).toFixed(2)}</span>
                      ) : <span className="text-slate-600">—</span>}
                    </td>
                    <td className="py-4 px-6 text-right">
                      <span className={`text-sm font-bold font-mono ${entry.balanceAfter >= 0 ? 'text-white' : 'text-amber-400'}`}>
