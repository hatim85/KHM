...
const Suppliers = () => {
  ...
  const openModal = (supplier = null) => {
    if (supplier) {
      ...
      setEditingId(null);
      setFormData({ name: '', gstin: '', phone: '', email: '', address: '', isActive: true });
    }
    setShowModal(true);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Manage your supplier database and GSTINs.</p>
        </div>
        <button
          onClick={() => openModal()}
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {loading && data.length === 0 ? (
                ...
                data.map((supplier) => (
                  <tr key={supplier._id} className="hover:bg-slate-800/20 transition">
                    <td className="py-4 px-6">
                      <p className="text-sm font-medium text-slate-200">{supplier.name}</p>
                    </td>
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${supplier.isActive ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}>
                        {supplier.isActive ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-right">
                      <Link to={`/ledger/supplier/${supplier._id}?name=${encodeURIComponent(supplier.name)}`} className="text-emerald-400 hover:text-emerald-300 text-sm font-medium mr-4 transition">Ledger</Link>
                      <button onClick={() => openModal(supplier)} className="text-indigo-400 hover:text-indigo-300 text-sm font-medium mr-4 transition">Edit</button>
                      <button onClick={() => handleDelete(supplier._id)} className="text-rose-400 hover:text-rose-300 text-sm font-medium transition">Delete</button>
                    </td>
                  </tr>
                ))
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl">
            <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
              <h2 className="text-lg font-bold text-white">{editingId ? 'Edit Supplier' : 'Add Supplier'}</h2>
              <button onClick={closeModal} className="text-slate-400 hover:text-white">&times;</button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Supplier Name *</label>
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                <label htmlFor="isActive" className="text-sm text-slate-300 font-medium">Active Supplier</label>
              </div>
