  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Chronological history of all stock changes across both streams.</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <select
          ...
          className="bg-slate-900 border border-slate-700 text-sm text-white rounded-xl px-4 py-2 outline-none appearance-none focus:border-indigo-500"
        >
        ...
        <select
          ...
          className="bg-slate-900 border border-slate-700 text-sm text-white rounded-xl px-4 py-2 outline-none appearance-none focus:border-indigo-500 max-w-xs"
        >
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="bg-slate-800/40 border-b border-slate-800">
                ...
                <th className="py-4 px-6 text-xs font-semibold text-slate-400 uppercase tracking-wider text-right">Quantity</th>
                <th className="py-4 px-6 text-xs font-semibold text-slate-400 uppercase tracking-wider">Source</th>
                <th className="py-4 px-6 text-xs font-semibold text-slate-400 uppercase tracking-wider">Remarks</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/50">
              {movementsLoading && movements.length === 0 ? (
                ...
                movements.map((m) => (
                  <tr key={m._id} className="hover:bg-slate-800/20 transition">
                    <td className="py-4 px-6 text-sm text-slate-300">
                      {new Date(m.createdAt).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })}
                    </td>
                    <td className="py-4 px-6">
                      <p className="text-sm font-medium text-slate-200">{m.product?.name || 'Unknown'}</p>
                      {m.product?.sku && <p className="text-xs text-slate-500 font-mono">{m.product.sku}</p>}
                    </td>
                    <td className="py-4 px-6">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${m.stream === 'TAX' ? 'bg-indigo-500/10 text-indigo-400' : 'bg-amber-500/10 text-amber-400'}`}>
                        {m.stream}
                      </span>
                    </td>
                    <td className="py-4 px-6">
                      <span className={`text-xs font-medium px-2 py-0.5 rounded ${
                        m.type === 'IN' ? 'bg-emerald-500/10 text-emerald-400' : 
                        m.type === 'OUT' ? 'bg-rose-500/10 text-rose-400' : 
                    ...
                    <td className="py-4 px-6 text-right">
                      <span className={`text-sm font-bold font-mono ${m.quantity > 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    ...
                    <td className="py-4 px-6">
                      <span className="text-xs text-slate-400 max-w-[200px] truncate block">{m.remarks || '—'}</span>
                    </td>
                  </tr>
                ))
              )}
