  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 dark:border-slate-800 pb-3">
        ...
        >
          <span className="flex items-center gap-1.5"><BoxesIcon size={15} /> Categories</span>
          <span className="text-xs px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-mono">
        ...
        >
          <span className="flex items-center gap-1.5"><TagIcon size={15} /> Brands</span>
          <span className="text-xs px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-mono">
            {brands.data.length}
          </span>
        </button>
        ...
        >
          <span className="flex items-center gap-1.5"><RulerIcon size={15} /> Units of Measure</span>
          <span className="text-xs px-2 py-0.5 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-mono">
            {units.data.length}
          </span>
        </button>
              ...
              {currentLoading && currentList.length === 0 ? (
                ...
                currentList.map((item) => (
                  <tr key={item._id} className="hover:bg-slate-100 dark:hover:bg-slate-800/20 transition">
                    ...
                    {activeTab === 'units' && (
                      <td className="py-4 px-6 text-sm font-mono text-indigo-600 dark:text-indigo-400">
                        {item.shortName}
                      </td>
                    )}
                    ...
                    <td className="py-4 px-6 text-right">
                      <button
                        ...
                        className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-300 text-sm font-medium mr-4 transition"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDelete(item._id, item.name)}
                        className="text-rose-600 dark:text-rose-400 hover:text-rose-300 text-sm font-medium transition"
                      >
                        Delete
                      </button>
                    </td>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              {activeTab === 'categories' && (
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-white dark:bg-slate-950/60 border border-slate-300 dark:border-slate-700/70 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white outline-none resize-none"
              ...
              {activeTab === 'units' && (
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-white dark:bg-slate-950/60 border border-slate-300 dark:border-slate-700/70 focus:border-indigo-500 rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white font-mono outline-none"
              ...
              <div className="flex items-center gap-3 pt-2">
                <input
                  ...
                  className="w-4 h-4 rounded bg-slate-100 dark:bg-slate-950 border-slate-300 dark:border-slate-700 text-indigo-600 focus:ring-indigo-500"
                />
                <label htmlFor="isActiveToggle" className="text-sm font-medium text-slate-600 dark:text-slate-300 select-none">
                  Active in System
                </label>
              </div>
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-slate-200 dark:border-slate-800">
                <button
                  type="button"
                  onClick={closeModal}
                  className="px-5 py-2 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition"
                >
                  Cancel
                </button>
                <button
