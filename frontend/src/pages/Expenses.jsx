          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="bg-slate-100 dark:bg-slate-800/40 border-b border-slate-200 dark:border-slate-800">
                ...
                <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Payment Mode</th>
                <th className="py-4 px-6 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Notes/Ref</th>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800/50">
              {expensesLoading && expenses.length === 0 ? (
                ...
                expenses.map((expense) => (
                  <tr key={expense._id} className="hover:bg-slate-100 dark:hover:bg-slate-800/20 transition">
                    <td className="py-4 px-6 text-sm text-slate-900 dark:text-white">
                      {new Date(expense.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </td>
                    <td className="py-4 px-6 text-sm font-medium text-indigo-600 dark:text-indigo-400">
                    ...
                    <td className="py-4 px-6">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300">
                        {expense.paymentMode.replace('_', ' ')}
                      </span>
                    </td>
                    <td className="py-4 px-6 text-sm text-slate-500 dark:text-slate-400 max-w-[200px] truncate">
                      {expense.referenceNumber && <span className="text-slate-600 dark:text-slate-300 font-mono text-xs mr-2">#{expense.referenceNumber}</span>}
                      {expense.notes}
                    </td>
                    <td className="py-4 px-6 text-right">
                      <button onClick={() => handleDeleteExpense(expense._id)} className="text-rose-600 dark:text-rose-400 hover:text-rose-300 text-sm font-medium transition">Delete</button>
                    </td>
                  </tr>
                ))
              )}
